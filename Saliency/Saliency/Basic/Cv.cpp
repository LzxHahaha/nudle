...
/************************************************************************/
void Cv::AbsAngle(CMat& cmplx32FC2, Mat& mag32FC1, Mat& ang32FC1)
{
...
/************************************************************************/
void Cv::GetCmplx(CMat& mag32F, CMat& ang32F, Mat& cmplx32FC2)
{
...
void Cv::Mat2GrayLog(CMat& img, Mat& newImg)
{
...
Rect Cv::GetMaskRange(CMat &mask1u, int ext, int thresh)
{
...
int Cv::GetRegions(const Mat_<byte> &label1u, Mat_<int> &regIdx1i, vecI &idxCount, vecB &idxLabel, bool noZero)
{
...
int Cv::GetNZRegions(const Mat_<byte> &label1u, Mat_<int> &regIdx1i, vecI &idxSum)
{
	vector<pair<int, int>> counterIdx;
	int _w = label1u.cols, _h = label1u.rows, maxIdx = -1;
	regIdx1i.create(label1u.size());
...
Mat Cv::GetNZRegionsLS(CMat &mask1u, double ignoreRatio)
{
	...
	vecI idxSum;
	Mat resMask;
	Cv::GetNZRegions(mask1u, regIdx1i, idxSum);
	if (idxSum.size() >= 1 && idxSum[0] > ignoreRatio)
		compare(regIdx1i, 0, resMask, CMP_EQ);
	return resMask;
}

int Cv::GetBorderPnts(Size sz, double ratio, vector<Point> &bdPnts)
{
...
Mat Cv::GetBorderRegC(CMat &img3u, Mat &idx1i, vecI &idxCount)
{
	Mat img, edgC;
	...
	Mat_<int> idx1iT;
	int regNum = Cv::GetRegions(edg, idx1iT, idxCount, vecB(), true);
	while (regNum > 1 && idxCount[regNum - 1] < 300)
		regNum--;
	Mat bdCMask = Cv::GetBorderReg(idx1iT, regNum), ignoreMask;
...
Mat Cv::GetBorderReg(CMat &idx1i, int regNum, double ratio, double thr)
{
	// Variance of x and y
	...
	Mat bReg1u = Mat::zeros(idx1i.size(), CV_8U);
...
void Cv::fillPoly(Mat& img, const vector<PointSeti> _pnts, const Scalar& color, int lineType, int shift, Point offset)
{
...
//void Cv::NormalizeImg(CStr &inDir, CStr &outDir, int minLen, bool subFolders)
//{
//	CmFile::MkDir(outDir);
//	vecS names, subFold;
...
void Cv::Demo(const char* fileName/* = "H:\\Resize\\cd3.avi"*/)
{

}

void Cv::AddAlpha(CMat &fg3u, CMat &alpha1u, Mat &res3u)
{
...
void Cv::AddAlpha(CMat &bg3u, CMat &fg3u, CMat &alpha1u, Mat &res3u)
{
...
void Cv::avgPerRegion(CMat &regIdx1i, Mat &unaryNf, int regNum)
{
...
void Cv::CannySimpleRGB(CMat &img3u, Mat &edge1u, double thresh1, double thresh2, int apertureSize, bool L2gradient)
{
	Mat bgr[3], edgTmp;
	split(img3u, bgr);
...
void Cv::rubustifyBorderMask(Mat& mask1u)
{
	Mat_<int> regIdx1i;
	int regNum = Cv::GetRegions(mask1u, regIdx1i, vecI(), vecB(), true);
	mask1u = Cv::GetBorderReg(regIdx1i, regNum, 0.02, 0.5);
}

int Cv::intMatMax(CMat idx1i)
{
	int maxV = -INT_MAX;
...
Mat Cv::getGrabMask(CMat &img3u, Rect rect)
{
	...
	Mat edge1u;
	Cv::CannySimpleRGB(img3u, edge1u, 120, 1200, 5);
