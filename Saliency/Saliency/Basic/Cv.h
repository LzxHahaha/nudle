...
Size Cv::NormalizeSize(const Size& sz, int shortLen, int unitLen)
{
...
void Cv::NormalizeImg(CMat&img, Mat& dstImg, int shortLen, int unitLen)
{
	resize(img, dstImg, NormalizeSize(img.size(), shortLen, unitLen));
}

void Cv::CheckPoint(Point &p, Size imgSz)
{
...
Rect Cv::GetImgRange(Point p1, Point p2, Size imgSz)
{
	CheckPoint(p1, imgSz);
	CheckPoint(p2, imgSz);
...
Mat Cv::Merge(CMat &m3c, CMat &m1c)
{
	Mat m4c;
	vecM chs;
	cv::split(m3c, chs);
...
void Cv::Split(CMat &m4c, Mat &m3c, Mat &m1c)
{
	vecM chs;
	cv::split(m4c, chs);
	m1c = chs[3];
...
template<class T> void Cv::PyrDownSample(CMat &src, Mat &dst)
{
...
template<class T> void Cv::PyrUpSample(CMat &src, Mat &dst, Size sz)
{
...
void Cv::SaveImgRGB(CStr &fName, CMat &img)
{
	Mat saveImg;
...
template <typename T> Mat Cv::addChannel(CMat &mat, int num, double defaultVal)
{
